...
def agent_run(pref,evals=20000,
              methods=[ACH_solution,rNSGAII_solution],
              problems=['DTLZ1','DTLZ2','DTLZ3','DTLZ4']):
    
    return Parallel(n_jobs=2)(
       delayed(agent_solve_problems)(target,pref,evals) 
...
def agent_paraller(target,preference,evals=20000):
    method=target[0]
    problem=target[1]

    return agent_solve(method,problem,preference,evals)

def agent_solve(method,problem,preference,evals=20000):
    ...
    objs=[]
    refs=[ref[:]]
    while ref is not None and i<10:
        objs.append(method(problem,ref,evals=evals))
